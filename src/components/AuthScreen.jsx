  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Enforce quad name (Arabic or English)
        const nameParts = formData.name.trim().split(/\s+/).filter(part => part.length >= 2);
        if (nameParts.length < 4) {
          throw new Error('يجب كتابة الاسم رباعياً بالكامل باللغة العربية أو الإنجليزية (مثال: أحمد محمد عبدالله علي).');
        }
        ...
        if (fakeEmailDomains.some(domain => emailDomain.includes(domain))) {
          throw new Error('الرجاء استخدام البريد الإلكتروني الحقيقي، يمنع النظام الإيميلات الوهمية.');
        }
      ...
      {showForgotPassword && (
        <div className="fixed inset-0 bg-slate-950/85 dark:bg-black/95 z-[9999] flex items-center justify-center p-4 backdrop-blur-lg transition-all duration-500 fade-in select-none">
          <div className="bg-white/95 dark:bg-[#09171a]/95 border-2 border-[#82af96] dark:border-[#3c6550] rounded-[24px] p-6 md:p-8 max-w-sm w-full shadow-2xl scale-in text-right relative overflow-hidden">
            ...
            {resetMessage && (
              <div className={`p-3 rounded-xl mb-4 text-xs font-black flex items-start gap-2 relative z-10 ${resetMessage.type === 'success'
